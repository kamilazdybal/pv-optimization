...
encoder_inputs = np.hstack((Yi_CS, mf/np.max(mf)))
...
for random_seed in random_seeds_list:
    ...
    model_weights_filename = 'f-PV-' + case_run_name + '-best-model-weights-rs-' + str(random_seed) + '.h5'
     
    if not os.path.exists('f-PV-' + case_run_name + '-MSE-training-losses-rs-' + str(random_seed) + '.csv'):
        ...
        input_layer = Input(shape=(n_variables,)) # it is equal to n_variables because there's n_variables - 1 mass fractions and a mixture fraction
        ...
        np.savetxt('f-PV-' + case_run_name + '-best-epoch-counter-rs-' + str(random_seed) + '.csv', ([best_epoch_counter]), delimiter=',', fmt='%.16e')
        np.savetxt('f-PV-' + case_run_name + '-previous-best-training-loss-rs-' + str(random_seed) + '.csv', ([previous_best_training_loss]), delimiter=',', fmt='%.16e')
        ...
        np.savetxt('f-PV-' + case_run_name + '-MSE-training-losses-rs-' + str(random_seed) + '.csv',(training_losses_across_epochs), delimiter=',', fmt='%.16e')
        np.savetxt('f-PV-' + case_run_name + '-MSE-validation-losses-rs-' + str(random_seed) + '.csv',(validation_losses_across_epochs), delimiter=',', fmt='%.16e')
