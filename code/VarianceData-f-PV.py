...
for random_seed in random_seeds_list:
    ...
    variance_data_file = 'f-PV-' + case_run_name + '-VD-for-' + '-'.join(VD_target_variables_names) + '-PV-source-rs-' + str(random_seed) + '.pkl'
    
    if not os.path.exists(variance_data_file):
    
        if os.path.exists('f-PV-' + case_run_name + '-MSE-training-losses-rs-' + str(random_seed) + '.csv'):
